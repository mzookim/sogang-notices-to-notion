...
- 공지 목록은 API를 우선 사용하고, 필요할 때 HTML/HTTP 보완 조회와 Playwright 기반 수집으로 보완한다.
...
GitHub Actions 워크플로는 Python `3.11` 기준으로 실행한다.
...
`본문 해시`, `본문 미디어 상태`, `첨부 상태`는 동기화 안정성을 위한 내부 관리 속성이다. 기본 데이터베이스 뷰에서는 숨겨 두고, 수동으로 수정하지 않는 것을 권장한다.
